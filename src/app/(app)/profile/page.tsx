...
function BaselineManager() {
  ...
  const [newSlotFoodQuery, setNewSlotFoodQuery] = useState('');
  const [newSlotFoodResults, setNewSlotFoodResults] = useState<Record<string, unknown>[]>([]);
  const [newSlotFood, setNewSlotFood] = useState<Record<string, unknown> | null>(null);
  ...
  const addSlot = async () => {
    ...
    // If a food was selected, save it first if external
    if (newSlotFood) {
      foodId = newSlotFood.id as number | null;
      if (!foodId) {
        const saveRes = await fetch('/api/foods', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...newSlotFood, is_custom: 0 }),
        });
    ...
    await fetch('/api/baseline', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ slot_name: newSlotName.trim(), default_food_id: foodId }),
    });
    setNewSlotName('');
    setNewSlotFood(null);
    setNewSlotFoodQuery('');
    setNewSlotFoodResults([]);
    fetchSlots();
  };

  const searchNewSlotFoods = async (q: string) => {
    setNewSlotFoodQuery(q);
    if (q.length < 2) { setNewSlotFoodResults([]); return; }
    const res = await fetch(`/api/foods?q=${encodeURIComponent(q)}`);
    const data = await res.json();
    if (data.success) setNewSlotFoodResults(data.data);
  };
